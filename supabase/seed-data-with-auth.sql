-- =====================================================
-- DATOS DE PRUEBA CON AUTENTICACIÓN - SAD LAS V2
-- =====================================================

-- =====================================================
-- ROLES DEL SISTEMA (solo si no existen)
-- =====================================================
...
-- =====================================================
-- CREAR USUARIOS EN AUTH.USERS (solo si no existen)
-- =====================================================

-- Nota: Estos usuarios se crearán con contraseñas temporales
-- Las contraseñas reales deberán ser cambiadas en el primer login

-- Super Administrador
DO $$
BEGIN
  -- Verificar si el usuario ya existe en auth.users
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'superadmin@sadlas.com') THEN
...
-- =====================================================
-- ADMINISTRADORES DE PRUEBA (solo si no existen)
-- =====================================================
...
-- =====================================================
-- TRABAJADORAS DE PRUEBA (solo si no existen)
-- =====================================================
...
-- =====================================================
-- ASIGNACIONES DE PRUEBA (solo si no existen)
-- =====================================================
...
(
  ...
  (SELECT id FROM workers WHERE email = 'carmen.fernandez@sadlas.com'),
  (SELECT id FROM users WHERE email = 'jose.perez@email.com'),
  'regular',
  '2025-01-01',
  null,
  8,
...
(
  ...
  (SELECT id FROM workers WHERE email = 'isabel.gonzalez@sadlas.com'),
  (SELECT id FROM users WHERE email = 'antonia.martinez@email.com'),
  'weekends',
  '2025-01-01',
  null,
  6,
...
(
  ...
  (SELECT id FROM workers WHERE email = 'rosa.lopez@sadlas.com'),
  (SELECT id FROM users WHERE email = 'francisco.rodriguez@email.com'),
  'holidays',
  '2025-01-01',
  null,
  5,
...
-- =====================================================
-- PLANES MENSUALES DE PRUEBA (solo si no existen)
-- =====================================================
...
-- =====================================================
-- DÍAS DE SERVICIO DE PRUEBA (solo si no existen)
-- =====================================================
...
-- =====================================================
-- ALERTAS DEL SISTEMA DE PRUEBA (solo si no existen)
-- =====================================================
...
UNION ALL
SELECT 'auth.users' as tabla, COUNT(*) as total FROM auth.users WHERE email LIKE '%@sadlas.com'
UNION ALL
...
-- Mostrar información de acceso para administradores
SELECT 
...
SELECT 
  'Super Admin:' as info,
  'superadmin@sadlas.com' as email,
  'TempPass123!' as password
UNION ALL
SELECT 
  'Admin:' as info,
  'admin@sadlas.com' as email,
  'TempPass123!' as password; 