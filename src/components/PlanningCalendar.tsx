  ...
  const detectConflicts = (day: WeekDay) => {
    ...
    for (let i = 0; i < dayAssignments.length; i++) {
      for (let j = i + 1; j < dayAssignments.length; j++) {
        ...
        // Check if same worker has overlapping times
                    ...
                    return (
                      ...
                      >
                        {timeAssignments.map((assignment) => (
                          <div
                            key={assignment.id}
                            className={`absolute inset-1 rounded text-xs p-1 cursor-pointer hover:shadow-md transition-shadow ${assignment.color} border`}
                            style={{ 
